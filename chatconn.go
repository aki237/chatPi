...
CONNLOOP:
	for {
		...
		case "JOIN":
			if len(command) != 3 {
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", SYNERR_JOIN, "error")))
				continue
			}
			if !authed {
				nick := command[1]
				...
				case user.ErrNickAlreadyTaken:
					c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", "Username already Taken", "error")))
					continue
				case nil:
					c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", u.Cookie, "cookie")))
					authed = true
					continue
				}
			} else {
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", "You have already been registered", "error")))
				continue
			}
		case "LIST":
			if len(command) != 2 {
				log(command, " : Wrong SYNTAX")
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", SYNERR_LIST, "error")))
				continue
			}
			if command[1] != "USERS" {
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", SYNERR_LIST, "error")))
			...
			c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", user.Ulistxml(), "userlist")))
			continue
		case "MSG":
			if !authed {
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", "You have to register first", "error")))
				continue
			}
			if len(command) < 6 ||
				command[1] != "WITH" ||
				command[3] != "TO" {
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", SYNERR_MSG, "error")))
				continue
			}
			if u.Cookie != command[2] {
				log(u.Cookie, command[2])
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", "Error : Cookie wrong", "error")))
				continue
			}
			reciever, err := user.GetUser(command[4])
			if err != nil {
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", "User not found. Use 'LIST USERS' to view the user list", "error")))
				continue
			}
		...
		case "BROADCAST":
			...
			if len(command) < 4 || command[1] != "WITH" {
				log("ERROR WITH \"WITH\"")
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", SYNERR_BCST, "error")))
				continue
			}
			if u.Cookie != command[2] {
				log(u.Cookie, command[2])
		...
		case "OUT":
			if authed {
				c.Write([]byte(user.FormMessageXML("private", "*ChatPi*", "Bye have a great time....", "message")))
				user.RemoveUser(u)
			}
			break CONNLOOP
		default:
